...
export default function ClientTracker() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const [c, p, q, pif] = await Promise.all([
          dbGet("clients"),
          dbGet("payments"),
          dbGet("quotes"),
          dbGet("pifClients"),
  ...
  function getPayData(cid) {
    return payments[getPKey(cid, viewMonth, viewYear)] || { status:"pending", amountPaid:0 };
  }
  ...
  function logPayment(cid, amount) {
    ...
    savePayments({ ...payments, [key]: { status:"paid", amountPaid: newPaid } });
    setPayInput(p => ({ ...p, [cid]:"" }));
  }
  function resetPay(cid) {
    const key = getPKey(cid, viewMonth, viewYear);
    savePayments({ ...payments, [key]: { status:"pending", amountPaid:0 } });
  }
  function totalPaidAllTime(cid) {
    return Object.entries(payments)
      .filter(([k]) => k.startsWith(⁠ pay_${cid}_ ⁠))
      .reduce((s,[,v]) => s+(parseFloat(v.amountPaid)||0), 0);
  }
  function getRemaining(c) {
    const q = parseFloat(c.quotedTotal);
    if (!q) return null;
    return Math.max(0, q - totalPaidAllTime(c.id));
  }

  function addClient() {
    if (!newClient.name.trim()) return;
    saveClients([...clients, { id:genId(), ...newClient, createdAt:new Date().toISOString() }]);
    setNewClient(blankClient); setShowAddClient(false);
  }
  function addPIF() {
    if (!newPIF.name.trim()) return;
    savePifClients([...pifClients, { id:genId(), ...newPIF, createdAt:new Date().toISOString() }]);
    setNewPIF(blankPIF); setShowAddPIF(false);
  }
  ...
  function removeQuote(id) {
    const key = getQKey(viewMonth, viewYear);
    saveQuotes({ ...quotes, [key]: (quotes[key]||[]).filter(q => q.id!==id) });
  }
  ...
  function exportPayments() {
    ...
    const rows = allRows.map(c => {
      ...
      return { "Name":c.name,"Company":c.company||"","Monthly Amount":c.monthlyAmount||"","Total Quoted":c.quotedTotal||"","Due Day":c.dueDay||"","Notes":c.notes||"","Status":c.status,"Amount Paid This Month":c.amountPaid||0,"Total Paid All Time":totalPaidAllTime(c.id),"Remaining Balance":rem!==null?rem:"" };
    });
    downloadCSV(⁠ payments_${MONTHS[viewMonth]}_${viewYear}.csv ⁠, rows, headers);
  }
  function exportPIF() {
    const headers = ["Name","Company","Amount","Date Paid","Notes"];
    const rows = sortedPIF.map(c => ({ "Name":c.name,"Company":c.company||"","Amount":c.amount||"","Date Paid":c.paidDate||"","Notes":c.notes||"" }));
    downloadCSV(⁠ pay_in_full_clients.csv ⁠, rows, headers);
  }
  function exportQuotes() {
    const headers = ["Month","Year","Name","Company","Amount","Notes"];
    const rows = [];
    Object.entries(quotes).forEach(([key,qs]) => {
      const parts=key.replace("quotes_","").split("_");
      (qs||[]).forEach(q => rows.push({ "Month":MONTHS[parseInt(parts[1])]||parts[1],"Year":parts[0],"Name":q.name,"Company":q.company||"","Amount":q.amount||"","Notes":q.notes||"" }));
    });
  ...
  const allRows = useMemo(() => {
    return clients
      .map(c => {
        const raw    = getPayData(c.id);
        const status = effectiveStatus(c, raw.status, viewMonth, viewYear);
  ...
  if (loading) return (
    <div style={{ fontFamily:"'DM Mono',monospace", background:"#080c10", minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", color:"#334155" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:32, letterSpacing:"0.15em", color:"#1e3a5f", marginBottom:8 }}>CLIENT TRACKER</div>
    ...
    <div style={{ fontFamily:"'DM Mono','Courier New',monospace", background:"#080c10", minHeight:"100vh", color:"#e2e8f0" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .modal{background:#0d1117;border:1px solid #1e293b;border-radius:8px;padding:24px;width:420px;max-height:90vh;overflow-y:auto}
      ...
      <div style={{ background:"#0a0e14", borderBottom:"1px solid #1e293b", padding:"16px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <div style={{ fontFamily:"'Bebas Neue'", fontSize:28, letterSpacing:"0.12em" }}>CLIENT TRACKER</div>
          <div style={{ fontSize:10, color:"#64748b", letterSpacing:"0.15em", marginTop:-2 }}>PAYMENTS · QUOTES · BALANCES</div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <button className="btn" onClick={()=>shiftMonth(-1)} style={{ background:"#1e293b", color:"#94a3b8", padding:"6px 12px", borderRadius:4 }}>◀️</button>
      ...
      <div style={{ display:"flex", borderBottom:"1px solid #1e293b", background:"#0a0e14" }}>
        {[["payments","PAYMENTS"],["pif","PAY IN FULL"],["quotes","QUOTES"]].map(([k,label])=>(
          <button key={k} className="btn" onClick={()=>setTab(k)} style={{ background:"none", color:tab===k?"#3b82f6":"#64748b", padding:"12px 20px", borderBottom:tab===k?"2px solid #3b82f6":"2px solid transparent", fontSize:11, letterSpacing:"0.12em" }}>{label}</button>
        ))}
        <div style={{ flex:1 }} />
        ...
        {tab==="pif" && <><button className="btn" onClick={exportPIF} style={{ background:"#1e293b", color:"#94a3b8", padding:"10px 14px", margin:"6px 0 6px 12px", borderRadius:4 }}>↓ CSV</button><button className="btn" onClick={()=>setShowAddPIF(true)} style={{ background:"#065f46", color:"#34d399", padding:"10px 18px", margin:"6px 12px", borderRadius:4 }}>+ ADD PIF CLIENT</button></>}
        ...
        <div style={{ padding:"20px 24px" }}>
              ...
              <div style={{ background:"#0a0f1a", border:"1px solid #1e3a5f", borderRadius:6, padding:"14px 18px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                  <div>
                    ...
                    <div style={{ fontSize:10, color:"#334155", marginTop:3 }}>{allRows.length} client{allRows.length!==1?"s":""} on payment plans</div>
                  </div>
              ...
              <div style={{ background:"#0a0f0a", border:"1px solid #1a3a1a", borderRadius:6, padding:"14px 18px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                  <div>
                    ...
                    <div style={{ fontSize:10, color:"#334155", marginTop:3 }}>across all active payment plans</div>
                  </div>
                  <div style={{ textAlign:"right" }}>
          ...
          {clients.length===0 ? (
            <div style={{ textAlign:"center", padding:"60px 0", color:"#334155" }}>
              ...
              <div style={{ fontSize:12, letterSpacing:"0.1em" }}>NO CLIENTS YET — ADD YOUR FIRST ONE</div>
            </div>
          ) : (
            <div style={{ border:"1px solid #1e293b", borderRadius:6, overflow:"hidden" }}>
              ...
              {filtered.map((c,i) => {
                const allTimePaid = totalPaidAllTime(c.id);
                const remaining   = getRemaining(c);
                const quoted      = parseFloat(c.quotedTotal)||0;
                const pct         = quoted>0?Math.min(100,(allTimePaid/quoted)*100):0;
                const isExp       = expandedId===c.id;
                const pData       = getPayData(c.id);
                const isMissed    = c.status==="missed";
                const dueDay      = parseInt(c.dueDay)||null;
                const isPast      = isCurrentMonth && dueDay && TODAY_DAY > dueDay;
                const isToday     = isCurrentMonth && dueDay && TODAY_DAY === dueDay;
                  ...
                  <div key={c.id} style={{ borderTop:i===0?"none":"1px solid #1e293b20" }}>
                    <div style={{ display:"grid", gridTemplateColumns:"20px 1.2fr 1fr 80px 80px 90px 80px 195px 28px", padding:"11px 14px", alignItems:"center", background:isMissed?(i%2===0?"#130808":"#150909"):(i%2===0?"#080c10":"#090d13"), gap:8, cursor:"pointer", borderLeft:isMissed?"3px solid #ef444460":"3px solid transparent" }} onClick={()=>setExpandedId(isExp?null:c.id)}>
                      ...
                      <div>
                        ...
                        {c.notes&&<div style={{ fontSize:10, color:"#475569", marginTop:1, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:140 }}>{c.notes}</div>}
                      </div>
