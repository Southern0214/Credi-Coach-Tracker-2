...
export default function ClientTracker() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const [c, p, q, pif, comp] = await Promise.all([
          dbGet("clients"), dbGet("payments"), dbGet("quotes"), dbGet("pifClients"), dbGet("completedClients"),
        ]);
        if (c)    setClients(c);
        if (p)    setPayments(p);
        if (q)    setQuotes(q);
  ...
  function logPayment(cid, amount) {
    ...
    if (client) {
      ...
      if (total > 0 && allPaid >= total) {
        const newCompleted = [...completedClients, { ...client, completedAt: new Date().toISOString(), totalCollected: allPaid }];
        saveCompletedClients(newCompleted);
  ...
  function totalPaidAllTime(cid) {
    return Object.entries(payments).filter(([k]) => k.startsWith(`pay_${cid}_`)).reduce((s,[,v]) => s+(parseFloat(v.amountPaid)||0), 0);
  }
  ...
  function moveToCompleted(cid) {
    const client = clients.find(c => c.id===cid);
    if (!client) return;
    const totalCollected = totalPaidAllTime(cid);
    saveCompletedClients([...completedClients, { ...client, completedAt: new Date().toISOString(), totalCollected }]);
    saveClients(clients.filter(c => c.id!==cid));
  }
  ...
  function restoreCompleted(id) {
    const c = completedClients.find(c => c.id===id);
    if (!c) return;
    const { completedAt, totalCollected, ...rest } = c;
    saveClients([...clients, rest]);
    saveCompletedClients(completedClients.filter(c => c.id!==id));
  }
  ...
  function exportPIF() {
    ...
    downloadCSV(`pay_in_full_clients.csv`, sortedPIF.map(c => ({ "Name":c.name,"Company":c.company||"","Amount":c.amount||"","Date Paid":c.paidDate||"","Notes":c.notes||"" })), headers);
  }
  ...
  const allRows = useMemo(() => {
    return clients
      .filter(c => isClientActiveInMonth(c, viewMonth, viewYear))
      .map(c => {
  ...
  return (
    <div style={{ fontFamily:"'DM Mono','Courier New',monospace", background:"#080c10", minHeight:"100vh", color:"#e2e8f0" }}>
      ...
      <div style={{ display:"flex", borderBottom:"1px solid #1e293b", background:"#0a0e14", overflowX:"auto" }}>
        {[["payments","PAYMENTS"],["completed","COMPLETED"],["pif","PAY IN FULL"],["quotes","QUOTES"]].map(([k,label])=>(
          <button key={k} className="btn" onClick={()=>setTab(k)} style={{ background:"none", color:tab===k?"#3b82f6":"#64748b", padding:"12px 16px", borderBottom:tab===k?"2px solid #3b82f6":"2px solid transparent", fontSize:11, letterSpacing:"0.1em", whiteSpace:"nowrap" }}>{label}</button>
        ))}
        <div style={{ flex:1 }} />
                  ...
                  <div key={c.id} style={{ borderTop:i===0?"none":"1px solid #1e293b20" }}>
                    <div style={{ display:"grid", gridTemplateColumns:"20px 1.2fr 1fr 80px 80px 90px 80px 195px 28px", padding:"11px 14px", alignItems:"center", background:isMissed?(i%2===0?"#130808":"#150909"):(i%2===0?"#080c10":"#090d13"), gap:8, cursor:"pointer", borderLeft:isMissed?"3px solid #ef444460":"3px solid transparent" }} onClick={()=>setExpandedId(isExp?null:c.id)}>
                      ...
                      <div>
                        ...
                        {numMonths&&<div style={{ fontSize:9, color:"#334155", marginTop:1 }}>{numMonths} month plan</div>}
                      </div>
      ...
      {tab==="completed" && (
        <div style={{ padding:"20px 24px" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:20 }}>
            ...
            <div style={{ background:"#051a10", border:"1px solid #134d32", borderRadius:6, padding:"12px 20px" }}><div style={{ fontSize:11, color:"#34d399", lineHeight:1.6, marginTop:4 }}>Clients move here automatically when fully paid off. They no longer appear in monthly projections.</div></div>
          </div>
